...
function DocumentConversion(options, comments) {
    ...
    function htmlPathForParagraph(element, messages) {
        ...
        if (style) {
            var path = style.to;
            if (element.numbering && element.numbering.isOrdered && element.numbering.numFmt) {
                var olType = NUM_FMT_TO_OL_TYPE[element.numbering.numFmt];
                if (olType) {
                    path = addOlTypeToPath(path, olType);
                }
            }
            return path;
        } else {
    ...
    function addOlTypeToPath(htmlPath, olType) {
        var modifiedElements = htmlPath._elements.map(function(el) {
            if (el.tagNames && el.tagNames["ol"]) {
                return htmlPaths.element("ol", {"type": olType}, {fresh: el.fresh, separator: el.separator});
            }
            return el;
        });
